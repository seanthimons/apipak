schema_version_acceptance <- function() {
  ...
  for (version in c('3.0.3', '3.1.0', '2.0')) {
    for (shape in list(scalar_array, object_array)) {
      ...
      parsed <- wrapmaint::read_operations(file)
      ...
      eval(
        parse(
          text = wrapmaint::render_operation(
            operation,
      ...
      fixtures <- wrapmaint::operation_fixtures(parsed$operations)
      stopifnot(
  ...
  unsupported <- jsonlite::fromJSON(
    system.file('catalogue/schema.json', package = 'wrapmaint'),
