schema_reports_acceptance <- function() {
  ...
  put(fixture, old)
  put(fixture, new)
  stopifnot(!length(specmill::schema_diff(old, new)))
  ...
  put(fixture, new)
  changed <- specmill::schema_diff(old, new)
  stopifnot(
    length(changed) == 1L,
    nrow(changed[[1L]]$modified) > 0L,
    specmill::count_diff_changes(changed)$breaking > 0L,
    grepl('requires review', specmill::format_diff_markdown(changed))
  )
  unlink(file.path(new, 'sample-prod.json'))
  put(fixture, new, 'sample-dev.json')
  removed <- specmill::schema_diff(old, new, stage_priority = 'prod')
  stopifnot(nrow(removed[[1L]]$removed) == 4L, nrow(removed[[1L]]$added) == 0L)
  put(fixture, new, 'new-domain-prod.json')
  added <- specmill::schema_diff(
    old,
    new,
    pattern = '-prod[.]json$',
    stage_priority = 'prod'
  ...
  error <- tryCatch(specmill::schema_diff(old, new), error = identity)
