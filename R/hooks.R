  ...
  for (fn_name in names(hook_config)) {
    ...
    if (!is.null(fn_config$transform)) {
      errors <- c(
        ...
        paste0(
          "Function ",
          fn_name,
          " configures unsupported hook stage 'transform'"
        )
      )
    }
    ...
    for (hook_type in c("pre_request", "post_response")) {
      if (!is.null(fn_config[[hook_type]])) {
        ...
        for (hook_fn in hook_names) {
          ...
          if (
            !exists(hook_fn, envir = hooks, mode = "function", inherits = FALSE)
          ) {
            errors <- c(
              errors,
              paste0(
    ...
    if (!is.null(fn_config$parameter_overrides) && isTRUE(wrapper$found)) {
      for (schema_name in names(fn_config$parameter_overrides)) {
        ...
        public_name <- if (is.null(override$name)) {
          make.names(schema_name)
        } else {
          override$name
        }
    ...
    if (!is.null(fn_config$request_template)) {
      ...
      if (length(helper_positions) == 0) {
        errors <- c(
          ...
          paste0(
            "Function ",
            fn_name,
            " does not call configured helper ",
            helper
          )
        )
      }
      ...
      if (
        !is.list(template$args) ||
          ...
          !all(nzchar(names(template$args)))
      ) {
        errors <- c(
          errors,
          paste0(
            "Function ",
            fn_name,
            " has an invalid request-template argument mapping"
          )
        )
      } else {
        ...
        for (argument_name in names(template$args)) {
          ...
          parsed <- tryCatch(parse(text = expression), error = function(error) {
            NULL
          })
          if (is.null(parsed)) {
            errors <- c(
