...
maintenance_disposition <- function(root) {
  ...
  target <- function(file) {
    ...
    if (grepl('unit_test_readiness_audit', file)) return(c('R/readiness.R', 'Client audit policy is specmill-readiness.yml; command binds installed functions.'))
    ...
    if (grepl('calculate_coverage', file)) return(c('R/coverage.R: coverage_report', 'Client partitions and badge paths are specmill-coverage.yml.'))
    ...
    if (grepl('stub_specs', file)) return(c('R/configuration.R; R/mappings.R; R/hooks.R', 'Selection, names, request mapping and docs move to apis/*.yml; chemistry expressions remain named dev/specmill_callbacks.R callbacks.'))
  ...
  for (i in seq_along(records)) {
    ...
    record$replacement <- if (record$kind != 'module entrypoint' &&
      exists(record$name, asNamespace('specmill'), inherits = FALSE)) {
      paste0('installed specmill namespace: ', record$name, '; ', replacement[[1L]])
    } else replacement[[1L]]
